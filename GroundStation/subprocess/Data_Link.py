...
def resolve_data(data_bytes):
    try:
        ...
        vars.label_9.config(text=data.get('ROLL'))
        vars.label_10.config(text=data.get('ROLL'))
        vars.label_11.config(text=data.get('ROLL'))
        vars.label_12.config(text=(data.get('Calibrated') == 0x3F and  '已校准' or '校准失败'))
